...
class BidictCache[K, V]:
    ...
    def save(self) -> None:
        self._cache_path.parent.parent.mkdir(parents=True, exist_ok=True)
    ...
    def load(self):
        ...
        try:
            with self._cache_path.open(mode="r", encoding="utf-8") as f:
                self._bidict = json.load(fp=f)
        except (json.JSONDecodeError, OSError, TypeError) as e:
            raise ValueError("Failure parsing cached JSON") from e
        except DuplicationError as e:
            raise ValueError("Duplicate key or value detected in cached JSON") from e
