...
class TasksHelper:
    ...
    def set_default_tasklist(self) -> None:
        ...
        # TODO: Decide whether to print id/path
        print(
    ...
    def list_tasklists(
        ...
        Args:
            n: Maximum number of task lists to display.
            show_ids: If True, include the task list IDs in the output.
        """
        items = self._fetch_tasklists()
        ...
        for idx, tlist in enumerate(items[:limit], start=1):
            ...
            id_suffix = f" (id: {list_id})" if show_ids else ""
            print(f"{idx}. {title}{default_suffix}{id_suffix}")
    ...
    def list_tasks(
        ...
        Args:
            tasklist_id: The ID of the task list to display tasks from.
            tasklist_title: The title of the task list to display tasks from.
                The corresponding ID is looked up in the cache.
            n: Maximum number of tasks to display.

        Note:
            Only one of ``tasklist_id`` or ``tasklist_title`` may be provided.
            If neither is provided, the default task list is used.
        """
        if tasklist_id is not None and tasklist_title is not None:
            print(
                "Cannot specify both --tasklist-id and --tasklist-title. "
                "Please use only one."
            )
            return
        ...
        if tasklist_title is not None:
            ...
            if effective_tasklist_id is None:
                print(
                    f"No task list found with title '{tasklist_title}'. "
                    "Use `gtasks lists` to see available task lists, or use --tasklist-id."
                )
                return
        else:
            effective_tasklist_id = tasklist_id or self._default_list_id
        ...
        # Get human-readable tasklist title from cache
        tasklist_display_name = (
            self._tasklist_cache.get_title(effective_tasklist_id)
            or effective_tasklist_id
        )
        ...
        if not items:
            print(f"No tasks found for task list '{tasklist_display_name}'.")
            return

        limit = n if n is not None else len(items)
        print(f"Showing first {limit} tasks from task list '{tasklist_display_name}':")
    ...
    def _fetch_tasklists(self) -> list[dict]:
        """Fetch all task lists from the API and update the cache.

        Returns:
            List of task list items from the API response.
        """
    ...
    def _resolve_tasklist_id_from_title(self, title: str) -> Optional[str]:
        ...
        Args:
            title: The tasklist title to look up.

        Returns:
            The tasklist ID if found, None otherwise.
        """
        tasklist_id = self._tasklist_cache.get_id(title)
        if tasklist_id is None:
            self._fetch_tasklists()
            tasklist_id = self._tasklist_cache.get_id(title)
        return tasklist_id
